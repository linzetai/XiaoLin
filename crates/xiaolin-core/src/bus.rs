...
pub fn signing_material(msg: &AgentMessage) -> XiaoLinResult<Vec<u8>> {
...
fn compute_hmac(secret: &[u8], msg: &AgentMessage) -> XiaoLinResult<[u8; 32]> {
    let material = signing_material(msg)?;
    let mut mac = HmacSha256::new_from_slice(secret)
        .map_err(|_| XiaoLinError::Config("bus HMAC key must be non-empty".to_string()))?;
    mac.update(&material);
    Ok(mac.finalize().into_bytes().into())
}

/// Sets [`AgentMessage::signature`] to hex-encoded HMAC-SHA256 of [`signing_material`].
pub fn sign_message(msg: &mut AgentMessage, secret: &[u8]) -> XiaoLinResult<()> {
...
impl MessageBus {
    ...
    pub fn sign_if_hmac(&self, msg: &mut AgentMessage) -> XiaoLinResult<()> {
    ...
    pub async fn send(&self, mut msg: AgentMessage) -> XiaoLinResult<()> {
        if msg.ttl == 0 {
            tracing::warn!(
                msg_id = %msg.id,
                from = %msg.from,
        ...
        if let Some(key) = self.hmac_key.as_deref() {
            if !verify_message(&msg, key) {
                ...
                return Err(XiaoLinError::BusInvalidSignature);
            }
            if let Ok(mut seen) = self.seen_ids.lock() {
                if seen.iter().any(|id| id == &msg.id) {
                    ...
                    return Err(XiaoLinError::Config(
            ...
            MessageTarget::Agent(ref target_id) => {
                ...
                if let Some(mailbox) = mailboxes.get(target_id) {
                    mailbox
                        ...
                        .send(msg)
                        .await
                        .map_err(|_| XiaoLinError::BusMailboxClosed)?;
                } else {
                    return Err(XiaoLinError::BusAgentNotFound(target_id.to_string()));
                }
            }
    ...
    pub async fn request(
        ...
        match tokio::time::timeout(timeout, rx).await {
            ...
            Ok(Err(_)) => {
                self.cleanup_pending(&msg.id).await;
                Err(XiaoLinError::BusReplyClosed)
            }
            Err(_) => {
                self.cleanup_pending(&msg.id).await;
                Err(XiaoLinError::BusRequestTimeout(timeout))
            }
        }
    }
...
mod tests {
    ...
    async fn bus_with_hmac_rejects_unsigned() {
        ...
        assert!(matches!(err, XiaoLinError::BusInvalidSignature));
