...
const DEFAULT_REGISTRY_URL: &str = "https://hub.xiaolin.dev/api/v1";
...
impl HubClient {
    ...
    /// Create a client that installs to the global ~/.xiaolin/skills/ directory.
    ...
    async fn search_github_fallback(
        ...
        let search_url = format!(
            "https://api.github.com/search/repositories?q={}+topic:xiaolin-skill&per_page={}",
            urlencoded(query),
            limit,
        );
...
mod tests {
    ...
    fn list_installed_empty() {
        let tmp = std::env::temp_dir().join("xiaolin_hub_test_empty");
    ...
    fn install_and_uninstall_local() {
        let tmp = std::env::temp_dir().join("xiaolin_hub_test_install");
        let _ = std::fs::remove_dir_all(&tmp);
