...
pub enum ConfigMode {
    /// Production: uses `~/.xiaolin/`
    Production,
    /// Development: uses `~/.xiaolin-dev/`
    Development,
    /// Named profile: uses `~/.xiaolin-<name>/`
    Profile(String),
}

/// Get the state directory path for the given config mode.
///
/// Returns `~/.xiaolin/` for production, `~/.xiaolin-dev/` for development,
/// or `~/.xiaolin-<name>/` for named profiles.
...
/// Intervals for gateway-hosted evolution background tasks ([`XiaoLinConfig::evolution`]).
///
...
/// Importance scoring weights — mirrors `xiaolin_memory::ImportanceScorer` fields
...
pub struct SecurityConfig {
    ...
    /// Default: `.xiaolin/exec-policy.toml` (project-level).
    #[serde(default)]
...
///
/// Search order (highest priority first): project `config/` > `~/.xiaolin/` > `~/.openclaw/`.
...
pub fn load_config(mode: &ConfigMode) -> XiaoLinResult<XiaoLinConfig> {
    let (xiaolin_paths, legacy_paths) = build_search_paths(mode);

    // Deep-merge all xiaolin-native config layers (project + user-level).
    // Iterate in reverse so highest-priority paths overlay lower ones.
    let mut base: Option<serde_json::Value> = None;
    for path in xiaolin_paths.iter().rev() {
        if !path.exists() {
            continue;
        }
                ...
                match try_repair_config(path) {
                    Ok(repaired) => {
                        let val: serde_json::Value =
                            json5::from_str(&repaired).map_err(XiaoLinError::json5)?;
                    ...
                    Err(_) => return Err(XiaoLinError::json5(e)),
                }
            }
        };
    ...
    for path in &legacy_paths {
        ...
        let loaded = (|| -> XiaoLinResult<XiaoLinConfig> {
            let text = std::fs::read_to_string(path)?;
            let raw: serde_json::Value = json5::from_str(&text).map_err(XiaoLinError::json5)?;
            let strict_includes = raw
                .get("strictIncludes")
                .and_then(|v| v.as_bool())
                .unwrap_or(false);
            ...
            warn_unknown_keys(&merged);
            let config: XiaoLinConfig = serde_json::from_value(merged)?;
            Ok(config)
        })();
        match loaded {
            ...
            Err(e) => {
                tracing::warn!(
                    path = %path.display(),
                    error = %e,
                    "legacy config ignored because it is incompatible with XiaoLin schema"
                );
            }
        }
    }
...
) -> XiaoLinResult<serde_json::Value> {
...
fn process_includes_inner(
    ...
    if depth > MAX_INCLUDE_DEPTH {
        return Err(XiaoLinError::config(format!(
        ...
        for p in paths {
            ...
            if inc_path.exists() {
                ...
                let inc_val: serde_json::Value =
                    json5::from_str(&inc_text).map_err(XiaoLinError::json5)?;
...
fn build_search_paths(mode: &ConfigMode) -> (Vec<PathBuf>, Vec<PathBuf>) {
    ...
    if let Some(home) = dirs::home_dir() {
        ...
        xiaolin.push(state_dir.join("config/default.json"));
    }
...
mod model_provider_config_tests {
    ...
    fn models_object_deserializes_to_typed_map_with_extra_fields() {
        ...
        let cfg: XiaoLinConfig = serde_json::from_value(j).expect("parse config");
    ...
    fn models_empty_object_defaults() {
        ...
        let cfg: XiaoLinConfig = serde_json::from_value(j).unwrap();
