...
/// (e.g. `~/.xiaolin/plugins/llm/<id>.json`).
...
/// Section inside `XiaoLinConfig` governing LLM provider plugins.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
mod tests {
    ...
    fn deserialize_middleware_plugin() {
        ...
        assert_eq!(mw.headers.get("x-app").unwrap(), "xiaolin");
