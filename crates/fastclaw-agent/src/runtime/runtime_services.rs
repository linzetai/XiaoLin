...
impl RuntimeServices {
    ...
    pub async fn fire_stop_hooks(
        ...
        assistant_messages: &[fastclaw_core::types::ChatMessage],
    ) {
        let Some(executor) = self.hooks.as_ref() else {
            return;
        };
    ...
    fn load_hooks(workspace_dir: Option<&Path>) -> Option<HookExecutor> {
        ...
        // Load from workspace .fastclaw/hooks.json
        if let Some(ws) = workspace_dir {
            let fastclaw_dir = ws.join(".fastclaw");
            if let Ok(cfg) = HookConfig::load_from_dir(&fastclaw_dir) {
                combined_config.merge(cfg);
            }
        }

        // Load from user home ~/.fastclaw/hooks.json
        if let Some(home) = dirs::home_dir() {
            let home_dir = home.join(".fastclaw");
    ...
    fn load_permissions(workspace_dir: Option<&Path>) -> Option<PermissionRuleEngine> {
        ...
        // Load from workspace .fastclaw/permissions.json
        if let Some(ws) = workspace_dir {
            let perm_path = ws.join(".fastclaw").join("permissions.json");
            if perm_path.exists() {
        ...
        // Load from user home ~/.fastclaw/permissions.json
        if let Some(home) = dirs::home_dir() {
            let perm_path = home.join(".fastclaw").join("permissions.json");
            if perm_path.exists() {
                if let Ok(content) = std::fs::read_to_string(&perm_path) {
                    if let Ok(rules) =
                        serde_json::from_str::<Vec<super::permissions::PermissionRule>>(&content)
