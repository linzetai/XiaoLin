...
impl SymbolIndex {
    ...
    pub fn index_file(
        &self,
        file_path: &str,
        symbols: &[fastclaw_treesitter::Symbol],
        file_hash: &str,
    ) -> anyhow::Result<()> {
...
pub fn start_background_scan(root: PathBuf, index: Arc<SymbolIndex>) {
    tokio::spawn(async move {
        ...
        for path in &walker {
            ...
            let parsed = match fastclaw_treesitter::CodeParser::parse(&source, &lang) {
                Ok(p) => p,
                Err(_) => continue,
            };

            let symbols = fastclaw_treesitter::extract_symbols(&parsed.tree, &source, &lang);
...
mod tests {
    ...
    fn create_and_lookup() {
        ...
        let symbols = vec![fastclaw_treesitter::Symbol {
            name: "process_data".into(),
            kind: fastclaw_treesitter::SymbolKind::Function,
            start_line: 10,
            end_line: 25,
            start_col: 1,
    ...
    fn incremental_reindex_replaces_old_symbols() {
        let (index, _tmp) = make_test_index();
        let v1 = vec![fastclaw_treesitter::Symbol {
            name: "old_fn".into(),
            kind: fastclaw_treesitter::SymbolKind::Function,
            start_line: 1,
            end_line: 5,
        ...
        let v2 = vec![fastclaw_treesitter::Symbol {
            name: "new_fn".into(),
            kind: fastclaw_treesitter::SymbolKind::Function,
            start_line: 1,
            end_line: 5,
    ...
    fn symbols_in_file_returns_ordered() {
        ...
        let symbols = vec![
            ...
            fastclaw_treesitter::Symbol {
                name: "alpha".into(),
                kind: fastclaw_treesitter::SymbolKind::Struct,
                start_line: 1,
                end_line: 10,
                start_col: 1,
