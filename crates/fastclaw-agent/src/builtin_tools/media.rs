...
impl Tool for TtsTool {
    ...
    fn description(&self) -> &str {
        "Call an OpenAI-compatible POST /audio/speech with model tts-1, synthesize speech from plain text, write an audio file on the gateway disk, and return JSON {path, bytes, voice}. \
         ...
         voice is one of alloy, echo, fable, onyx, nova, shimmer; default alloy. output_path sets the destination (mp3); omit for /tmp/fastclaw_tts_<timestamp>.mp3—pick a writable directory. \
    ...
    fn parameters_schema(&self) -> ToolParameterSchema {
        ...
        props.insert(
            ...
            serde_json::json!({
                "type": "string",
                "description": "Filesystem path for the written audio file (typically .mp3). Example: '/tmp/fastclaw_summary.mp3'. Parent directories are created when possible. Omit to use /tmp/fastclaw_tts_<timestamp>.mp3."
            }),
        );
        ToolParameterSchema {
            schema_type: "object".to_string(),
        ...
        let output_path = args
            ...
            .unwrap_or_else(|| {
                let ts = std::time::SystemTime::now()
                    ...
                    .map(|d| d.as_millis())
                    .unwrap_or(0);
                format!("/tmp/fastclaw_tts_{ts}.mp3")
            });
