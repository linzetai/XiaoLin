//! UDS-based proxy routing for bubblewrap network namespaces.
//!
//! Instead of relying on iptables/nftables (unavailable in many container
//! runtimes), this module uses Unix Domain Sockets as a bridge between the
//! host network namespace and the isolated sandbox namespace:
//!
//! 1. Host side: create UDS listeners, fork bridge processes that relay
//!    UDS ↔ TCP loopback.
//! 2. Sandbox side: fork local bridge processes that relay TCP loopback ↔ UDS,
//!    then rewrite proxy environment variables to point at the local bridges.
//!
//! This approach works in unprivileged containers and avoids requiring CAP_NET_ADMIN.
...
const PROXY_SOCKET_DIR_PREFIX: &str = "xiaolin-linux-sandbox-proxy-";
...
/// Prepare proxy routes on the host side.
///
/// Scans all proxy environment variables, extracts loopback endpoints,
/// creates UDS listeners in a temporary directory, and spawns bridge
/// processes (UDS ↔ TCP). Returns a serialized `ProxyRouteSpec` that
/// should be passed into the sandbox via `activate_proxy_routes_in_netns`.
pub fn prepare_host_proxy_route_spec() -> io::Result<String> {
...
/// Activate proxy routes inside the sandbox network namespace.
///
/// Spawns local bridge processes (TCP loopback ↔ UDS) and rewrites
/// the proxy environment variables to point at the local bridges.
pub fn activate_proxy_routes_in_netns(serialized_spec: &str) -> io::Result<()> {
    ...
    for route in spec.routes {
        ...
        // SAFETY: this runs in the single-threaded sandbox helper before exec.
        unsafe {
...
/// Legacy setup_proxy_routing for backward compatibility.
/// Prefers UDS bridging when proxy env vars are available,
/// falls back to iptables/nftables redirect when they are not.
pub fn setup_proxy_routing(proxy_port: u16) -> anyhow::Result<()> {
    tracing::info!("setting up proxy routing: redirecting to port {proxy_port}");

    run_cmd("ip", &["link", "set", "lo", "up"])?;

    if setup_nftables(proxy_port).is_ok() {
        tracing::info!("proxy routing: nftables redirect configured");
        return Ok(());
    }

    setup_iptables(proxy_port)?;
    tracing::info!("proxy routing: iptables redirect configured");
    Ok(())
}

fn setup_nftables(proxy_port: u16) -> anyhow::Result<()> {
    let nft = which::which("nft").map_err(|_| anyhow::anyhow!("nft not found"))?;
    let nft = nft.to_string_lossy();

    let rules = format!(
        "table ip nat {{\n\
           chain output {{\n\
             type nat hook output priority -100;\n\
             tcp dport != {proxy_port} redirect to :{proxy_port}\n\
           }}\n\
         }}"
    );
...
fn setup_iptables(proxy_port: u16) -> anyhow::Result<()> {
    let iptables = which::which("iptables")
        .map(|p| p.to_string_lossy().to_string())
        .unwrap_or_else(|_| "iptables".to_string());
...
fn run_cmd(program: &str, args: &[&str]) -> anyhow::Result<()> {
    let status = std::process::Command::new(program)
        .args(args)
        .status()
        .map_err(|e| anyhow::anyhow!("{program} failed to start: {e}"))?;
...
fn proxy_socket_parent_dir() -> PathBuf {
    if let Some(xiaolin_home) = std::env::var_os("XIAOLIN_HOME") {
        let candidate = PathBuf::from(xiaolin_home).join("tmp");
...
fn disable_process_dumping() -> io::Result<()> {
    let res = unsafe { libc::prctl(libc::PR_SET_DUMPABLE, 0) };
    if res != 0 {
        Err(io::Error::last_os_error())
    } else {
        Ok(())
...
fn proxy_bidirectional(mut tcp_stream: TcpStream, mut unix_stream: UnixStream) -> io::Result<()> {
    ...
    let tcp_to_unix = std::thread::spawn(move || io::copy(&mut tcp_reader, &mut unix_writer));
...
mod tests {
    ...
    fn cleanup_proxy_socket_dir_removes_bridge_artifacts() {
        ...
        let socket_dir = root.path().join("xiaolin-linux-sandbox-proxy-test");
        fs::create_dir(&socket_dir).expect("socket dir should create");
        let marker = socket_dir.join("bridge.sock");
        fs::write(&marker, b"test").expect("marker should write");
    ...
    #[test]
    fn proxy_route_spec_serialization() {
    ...
    fn parse_proxy_socket_dir_owner_pid_reads_owner_pid() {
        assert_eq!(
            parse_proxy_socket_dir_owner_pid("xiaolin-linux-sandbox-proxy-1234-0"),
            Some(1234)
        );
        assert_eq!(
            parse_proxy_socket_dir_owner_pid("xiaolin-linux-sandbox-proxy-x"),
            None
        );
    ...
    fn cleanup_stale_proxy_socket_dirs_removes_dead_pid_directories() {
        ...
        fs::create_dir(&dead_dir).expect("dead dir should create");

        let alive_dir = root
            .path()
            .join(format!("{PROXY_SOCKET_DIR_PREFIX}{}-1", std::process::id()));
        fs::create_dir(&alive_dir).expect("alive dir should create");

        let unrelated_dir = root.path().join("unrelated-proxy-dir");
        fs::create_dir(&unrelated_dir).expect("unrelated dir should create");
    ...
    #[test]
    fn rewrite_preserves_no_scheme_format() {
        let rewritten = rewrite_proxy_env_value("127.0.0.1:8080", 43210).unwrap();
        assert!(rewritten.contains("43210"));
        assert!(!rewritten.starts_with("http://"));
    }

    #[test]
    fn all_proxy_env_keys_covered() {
        assert!(PROXY_ENV_KEYS.len() >= 14);
        assert!(PROXY_ENV_KEYS.contains(&"HTTP_PROXY"));
        assert!(PROXY_ENV_KEYS.contains(&"DOCKER_HTTPS_PROXY"));
        assert!(PROXY_ENV_KEYS.contains(&"PIP_PROXY"));
    }

    #[test]
    fn loopback_localhost_parses() {
        let endpoint = parse_loopback_proxy_endpoint("http://localhost:9999");
        assert_eq!(
            endpoint,
