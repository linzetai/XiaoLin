...
impl SessionStore {
    ...
    fn parse_chat_messages_from_rows(
        ...
        for row in rows {
            let role = match row.role.as_str() {
                "system" => xiaolin_core::types::Role::System,
                "user" => xiaolin_core::types::Role::User,
                "assistant" => xiaolin_core::types::Role::Assistant,
                "tool" => xiaolin_core::types::Role::Tool,
                other => {
    ...
    pub async fn upsert_trace(
        &self,
        trace: &xiaolin_core::types::ConversationTrace,
    ) -> anyhow::Result<()> {
    ...
    pub async fn get_trace(
        &self,
        trace_id: &str,
    ) -> anyhow::Result<Option<xiaolin_core::types::ConversationTrace>> {
    ...
    pub async fn list_traces(
        ...
        limit: u32,
        offset: u32,
    ) -> anyhow::Result<Vec<xiaolin_core::types::ConversationTrace>> {
    ...
    /// This decouples `xiaolin-session` from `xiaolin-context`.
    ...
    pub async fn append_history_item(
        &self,
        session_id: &str,
        item: &xiaolin_protocol::HistoryItem,
    ) -> anyhow::Result<()> {
    ...
    pub async fn append_history_items(
        &self,
        session_id: &str,
        items: &[xiaolin_protocol::HistoryItem],
    ) -> anyhow::Result<()> {
        if items.is_empty() {
            return Ok(());
        }
...
fn extract_history_item_type(item: &xiaolin_protocol::HistoryItem) -> &'static str {
    match item {
        xiaolin_protocol::HistoryItem::Message { .. } => "message",
        xiaolin_protocol::HistoryItem::ToolUse { .. } => "tool_use",
        xiaolin_protocol::HistoryItem::CompactBoundary { .. } => "compact_boundary",
        xiaolin_protocol::HistoryItem::TurnUsage { .. } => "turn_usage",
        _ => "unknown",
    }
}
...
) -> anyhow::Result<xiaolin_core::types::ConversationTrace> {
    Ok(xiaolin_core::types::ConversationTrace {
        trace_id: r.0,
        session_id: r.1,
        agent_id: r.2,
        model: r.3,
...
mod tests {
    ...
    fn sample_trace(id: &str) -> xiaolin_core::types::ConversationTrace {
        xiaolin_core::types::ConversationTrace {
