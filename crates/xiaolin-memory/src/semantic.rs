...
mod tests {
    ...
    async fn search_multiword_across_fields() {
        ...
        // Multi-word: "XiaoLin" in subject, "version" in predicate
        let hits = m.search("XiaoLin version", 10).await.unwrap();
        assert_eq!(hits.len(), 1);
        assert_eq!(hits[0].id, "f1");

        // Single word still works
        let hits = m.search("XiaoLin", 10).await.unwrap();
        assert_eq!(hits.len(), 1);

        // Both words must match (AND semantics) — "vim" has no "XiaoLin"
        let hits = m.search("XiaoLin vim", 10).await.unwrap();
