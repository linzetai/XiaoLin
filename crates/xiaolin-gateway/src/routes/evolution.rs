...
pub(super) async fn submit_feedback(
    ...
    let kind = match body.kind.as_str() {
        "thumbs_up" => xiaolin_evolution::FeedbackKind::ThumbsUp,
        "thumbs_down" => xiaolin_evolution::FeedbackKind::ThumbsDown,
        "rating" => {
            let v = body.value.as_ref().and_then(|v| v.as_f64()).unwrap_or(3.0) as f32;
            xiaolin_evolution::FeedbackKind::Rating(v)
        }
        "correction" => {
            let text = body
                .value
                ...
                .unwrap_or("")
                .to_string();
            xiaolin_evolution::FeedbackKind::Correction(text)
        }
        other => {
...
pub(super) async fn evaluate_agent(
    ...
    let eval = xiaolin_evolution::StrategyEvaluator::new(&state.store.feedback_store);
