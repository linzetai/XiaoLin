...
async fn feishu_plugin_parses_text_message() {
    ...
    match plugin.handle_webhook(payload).await.unwrap() {
        WebhookResult::Messages(msgs) => {
            ...
            assert_eq!(msg.text, "Hello XiaoLin!");
...
async fn feishu_tool_validates_empty_args() {
    ...
    let client = std::sync::Arc::new(xiaolin_feishu::FeishuClient::new("t", "s"));
    let send_tool = xiaolin_feishu::FeishuSendMessageTool::new(client.clone());
    let reply_tool = xiaolin_feishu::FeishuReplyMessageTool::new(client.clone());
    let get_tool = xiaolin_feishu::FeishuGetChatMessagesTool::new(client);
...
async fn feishu_tool_rejects_invalid_json() {
    ...
    let client = std::sync::Arc::new(xiaolin_feishu::FeishuClient::new("t", "s"));
    let send_tool = xiaolin_feishu::FeishuSendMessageTool::new(client);
...
async fn feishu_messaging_inbound_parse() {
    use xiaolin_feishu::messaging::inbound::parse_message_event;
...
async fn feishu_messaging_mention_extraction() {
    use xiaolin_feishu::messaging::inbound::{extract_message_body, mentioned_bot};
    use xiaolin_feishu::messaging::MentionRef;
...
async fn feishu_event_handler_parsing() {
    use xiaolin_feishu::channel::{parse_event, FeishuEventType};
...
async fn feishu_core_types_brand() {
    use xiaolin_feishu::core::types::{FeishuProbeResult, LarkBrand};
...
async fn feishu_config_schema_valid() {
    use xiaolin_feishu::core::config_schema::feishu_config_json_schema;
...
async fn feishu_diagnose_format() {
    use xiaolin_feishu::commands::diagnose::{
