...
//! workflows against a copy of the XiaoLin source tree. They exercise the full
//! harness at realistic complexity (20–60+ tool calls per scenario).
//!
//! Run with:  `cargo test -p xiaolin-gateway --test e2e_real_llm -- --ignored --test-threads=1 --nocapture`
...
impl RealLlmServer {
    async fn start(work_dir: PathBuf) -> Self {
        let plugin_config_path = dirs::home_dir()
            .expect("home dir")
            .join(".xiaolin/plugins/llm/wps-llm-provider.json");
        assert!(
...
/// Create a git worktree of the XiaoLin repo for isolated testing.
...
fn seed_type_error(work_dir: &Path) -> SeedInfo {
    let file = work_dir.join("crates/xiaolin-agent/src/runtime/query_state.rs");
...
fn seed_logic_bug(work_dir: &Path) -> SeedInfo {
    let file = work_dir.join("crates/xiaolin-agent/src/runtime/context_compressor.rs");
...
async fn scenario_2_add_new_tool() {
    ...
    let prompt = format!(
        "你是一个 Rust 专家。请给这个项目新增一个 built-in tool，叫做 `checksum_tool`：\n\
         ...
         - 位置：在 crates/xiaolin-agent/src/builtin_tools/ 目录下新建 checksum.rs\n\
    ...
    let mod_file = work_dir.join("crates/xiaolin-agent/src/builtin_tools/mod.rs");
...
async fn scenario_3_refactor_extract_module() {
    ...
    let target_dir = e2e_target_dir();
    let prompt = format!(
        "你是一个 Rust 重构专家。请将 crates/xiaolin-agent/src/autofix.rs 中的 \
         `CompilerDiagnostic` struct 和相关的类型/函数提取到一个新的子模块中。\n\n\
         具体步骤：\n\
         1. 先读取 autofix.rs 了解文件结构\n\
         2. 创建目录 crates/xiaolin-agent/src/autofix/\n\
    ...
    let autofix_dir = work_dir.join("crates/xiaolin-agent/src/autofix");
...
async fn scenario_4_debug_failing_test() {
    ...
    let prompt = format!(
        "你是一个 Rust 调试专家。这个项目的一个单元测试失败了，测试输出如下：\n\
         ...
         项目根目录: {work_dir}\n\
         运行 cargo 命令时请加上 CARGO_TARGET_DIR={target_dir}\n\
         运行测试命令: CARGO_TARGET_DIR={target_dir} cargo test -p xiaolin-agent -- {test_filter} --nocapture",
        work_dir = work_dir.display(),
        target_dir = target_dir.display()
    );
...
async fn scenario_5_endurance_multi_turn() {
    ...
    let turns: Vec<String> = vec![
        format!("读取 {}/crates/xiaolin-core/src/config.rs 的前 80 行，告诉我 XiaoLinConfig 有哪些字段", work_dir.display()),
        "在 XiaoLinConfig struct 中新增一个字段 `pub e2e_test_marker: Option<String>`，加上 `#[serde(default)]` 属性".into(),
        format!("运行 {} cargo check --message-format=short 确认编译通过", cargo_prefix),
        "搜索项目中哪些文件使用了 XiaoLinConfig（用 grep 搜索）".into(),
        format!("读取 {}/crates/xiaolin-gateway/src/state/mod.rs 的前 50 行", work_dir.display()),
        "在 state/mod.rs 的 for_test 函数中，给 config 设置 e2e_test_marker = Some(\"harness-v1\".to_string())".into(),
        format!("运行 {} cargo check --message-format=short 确认编译通过", cargo_prefix),
        format!("读取 {}/crates/xiaolin-agent/src/runtime/query_state.rs 的前 40 行", work_dir.display()),
        "告诉我 QueryLoopState 结构体有多少个字段".into(),
        "在 QueryLoopState 中新增一个字段 `pub e2e_turn_count: u32`，并在 new() 中初始化为 0".into(),
        format!("运行 {} cargo check --message-format=short 确认编译通过", cargo_prefix),
        "回忆一下：我们在第 2 步给 XiaoLinConfig 添加了什么字段？".into(),
        format!("读取 {}/crates/xiaolin-core/src/config.rs 确认 e2e_test_marker 字段确实存在", work_dir.display()),
        "把 XiaoLinConfig 中 e2e_test_marker 的类型从 Option<String> 改为 Option<u32>".into(),
...
async fn scenario_6_task_decomposer() {
    ...
    let prompt = format!(
        "请完成以下复杂任务（多步骤）：\n\n\
         1. 读取 {work_dir}/crates/xiaolin-core/src/config.rs，了解 XiaoLinConfig 结构\n\
         2. 在 XiaoLinConfig 中新增一个字段 `pub decomposer_test_flag: Option<bool>`，加上 `#[serde(default)]`\n\
         3. 在 {work_dir}/crates/xiaolin-core/src/config.rs 的 impl Default 中初始化这个字段为 None\n\
         4. 运行 CARGO_TARGET_DIR={target_dir} cargo check --message-format=short 验证编译通过\n\
         5. 搜索整个项目中哪些文件 import 了 XiaoLinConfig\n\
         6. 汇总结果告诉我\n\n\
...
async fn scenario_7_validation_pipeline() {
    ...
    let prompt = format!(
        "请在项目目录 {work_dir} 中执行以下操作：\n\
         ...
         2. 运行 `wc -l crates/xiaolin-agent/src/runtime/mod.rs` 统计行数\n\
...
async fn scenario_8_undo_engine_rollback() {
    ...
    let target_file = work_dir.join("crates/xiaolin-agent/src/runtime/query_state.rs");
