//! End-to-end scenario tests for XiaoLin.
//!
...
async fn e2e_memory_lifecycle() {
    ...
    let _: Value = client
        .post(srv.url("/api/v1/chat"))
        .json(&json!({
            "messages": [{"role": "user", "content": "Explain XiaoLin memory"}],
            "stream": false
        }))
        .send()
        .await
...
async fn memory_recall_injects_relevant_memories() {
    ...
    let chat_resp: Value = client
        .post(srv.url("/api/v1/chat"))
        .json(&json!({
            "messages": [{"role": "user", "content": "What database does XiaoLin use for storage?"}],
            "stream": false,
            "agent_id": "main"
        }))
        .send()
...
async fn e2e_global_mcp_config_round_trip() {
    ...
    use xiaolin_core::config_access::{filter_config_for_read, CONFIG_WRITABLE_KEYS};
    ...
    let default_config: XiaoLinConfig = serde_json::from_value(json!({})).unwrap();
    assert!(
