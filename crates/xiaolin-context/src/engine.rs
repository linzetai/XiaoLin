...
impl ContextEngine {
    ...
    pub async fn process(&self, messages: &mut Vec<ChatMessage>) -> anyhow::Result<()> {
        let non_system = messages
            .iter()
            .filter(|m| !matches!(m.role, xiaolin_core::types::Role::System))
            .count();
...
impl SandboxAwarenessHook {
    ...
    pub fn detect() -> Self {
        let is_sandboxed =
            std::env::var("XIAOLIN_SANDBOX").is_ok() || std::env::var("SANDBOX").is_ok();
        let sandbox_type = std::env::var("XIAOLIN_SANDBOX")
...
impl PersonalityHook {
    ...
    pub fn from_workspace(workspace: &xiaolin_core::workspace::AgentWorkspace) -> Self {
        ...
        let user_path = workspace
            .root
            .join(xiaolin_core::workspace::DEFAULT_USER_FILENAME);
...
impl ContextHook for MemoryIngestHook {
    ...
    async fn on_ingest(
        ...
        let last_user_msg = input
            ...
            .iter()
            .rev()
            .find(|m| matches!(m.role, xiaolin_core::types::Role::User))
        ...
        if !memory_parts.is_empty() {
            ...
            let insert_pos = messages
                .iter()
                .position(|m| !matches!(m.role, xiaolin_core::types::Role::System))
                .unwrap_or(messages.len());
            messages.insert(
                insert_pos,
                ChatMessage {
...
impl AgentPersonalityHook {
    ...
    pub fn add_agent(
        &mut self,
        agent_id: &str,
        workspace: &xiaolin_core::workspace::AgentWorkspace,
    ) {
        self.workspaces
...
pub struct AgentMemoryIngestHook {
    episodic_map: std::collections::HashMap<String, Arc<xiaolin_memory::EpisodicMemory>>,
    semantic_map: std::collections::HashMap<String, Arc<xiaolin_memory::SemanticMemory>>,
    embedder: Option<Arc<dyn xiaolin_memory::EmbeddingProvider>>,
    max_snippets: usize,
}
