...
impl Default for PipelineConfig {
    fn default() -> Self {
        Self {
            enable_snip: env_var_is_true("XIAOLIN_ENABLE_SNIP", true),
            enable_micro_compact: env_var_is_true("XIAOLIN_ENABLE_MICROCOMPACT", true),
            enable_collapse: env_var_is_true("XIAOLIN_ENABLE_COLLAPSE", false),
            enable_auto_compact: env_var_is_true("XIAOLIN_ENABLE_AUTOCOMPACT", true),

            snip_max_tokens: env_var_parse("XIAOLIN_SNIP_MAX_TOKENS", 128_000),
            snip_min_rounds: env_var_parse("XIAOLIN_SNIP_MIN_ROUNDS", 3),

            micro_max_messages: env_var_parse(
                "XIAOLIN_MICRO_MAX_MESSAGES",
                DEFAULT_IMPORTANCE_MAX_MESSAGES,
            ),
            micro_recent_window: env_var_parse(
                "XIAOLIN_MICRO_RECENT_WINDOW",
                DEFAULT_IMPORTANCE_RECENT_WINDOW,
            ),

            reactive_target_tokens: env_var_parse("XIAOLIN_REACTIVE_TARGET_TOKENS", 128_000),
        }
    }
}
...
mod tests {
    ...
    fn integration_system_messages_survive_all_compaction() {
        let context_window = 128_000;
        let mut msgs = vec![
            sys("You are XiaoLin, a helpful AI assistant."),
