...
pub fn transform(
    ...
    env.insert("FASTCLAW_SANDBOXED".to_string(), "1".to_string());
...
/// Basename used when FastClaw self-invokes as the Linux sandbox helper.
pub const FASTCLAW_LINUX_SANDBOX_ARG0: &str = "fastclaw-linux-sandbox";

/// Attempt to auto-discover the `fastclaw-linux-sandbox` binary.
///
/// Search order:
...
pub fn discover_linux_sandbox_exe() -> Option<PathBuf> {
    if let Ok(current_exe) = std::env::current_exe() {
        if let Some(dir) = current_exe.parent() {
            let candidate = dir.join(FASTCLAW_LINUX_SANDBOX_ARG0);
    ...
    if let Ok(found) = which::which(FASTCLAW_LINUX_SANDBOX_ARG0) {
...
mod tests {
    use super::*;
    use fastclaw_path::AbsolutePathBuf;
    use fastclaw_security::{FileSystemSandboxEntry, FileSystemSpecialPath};
    ...
    fn transform_restricted_has_sandbox_setup() {
        ...
        assert!(cmd.env.contains_key("FASTCLAW_SANDBOXED"));
