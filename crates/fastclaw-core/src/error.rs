//! Structured errors for `fastclaw-core`.
//!
//! `FastClawError` implements [`std::error::Error`]. For call sites that still use
...
//! [`FastClawError`] (backward compatible; no extra `From` impl required).

use std::time::Duration;

use thiserror::Error;

/// Primary error type for `fastclaw-core` public APIs.
#[derive(Debug, Error)]
pub enum FastClawError {
...
pub type FastClawResult<T> = std::result::Result<T, FastClawError>;
