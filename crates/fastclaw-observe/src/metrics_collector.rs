...
impl MetricsCollector {
    ...
    pub fn render_prometheus(&self) -> String {
        ...
        for e in self.histograms.iter() {
            ...
            let _ = writeln!(
                &mut out,
                "fastclaw_endpoint_latency_ms{{endpoint=\"{}\",quantile=\"0.5\"}} {}",
                endpoint, p50
            );
            let _ = writeln!(
                &mut out,
                "fastclaw_endpoint_latency_ms{{endpoint=\"{}\",quantile=\"0.95\"}} {}",
                endpoint, p95
            );
            let _ = writeln!(
                &mut out,
                "fastclaw_endpoint_latency_ms{{endpoint=\"{}\",quantile=\"0.99\"}} {}",
                endpoint, p99
            );
            let _ = writeln!(
                &mut out,
        ...
        out.push_str("# HELP fastclaw_provider_requests_total Requests by provider and model\n");
        out.push_str("# TYPE fastclaw_provider_requests_total counter\n");
        for e in self.counters.iter() {
            let key = e.key();
            if let Some(rest) = key.strip_prefix("provider_request|") {
                ...
                if parts.len() == 2 {
                    let _ = writeln!(
                        &mut out,
                        "fastclaw_provider_requests_total{{provider=\"{}\",model=\"{}\"}} {}",
                        parts[0],
                        parts[1],
                        e.value().load(Ordering::Relaxed)
                    );
        ...
        out.push_str("# HELP fastclaw_provider_tokens_total Tokens by provider and model\n");
        out.push_str("# TYPE fastclaw_provider_tokens_total counter\n");
        for e in self.counters.iter() {
            let key = e.key();
            if let Some(rest) = key.strip_prefix("provider_tokens|") {
                ...
                if parts.len() == 2 {
                    let _ = writeln!(
                        &mut out,
                        "fastclaw_provider_tokens_total{{provider=\"{}\",model=\"{}\"}} {}",
                        parts[0],
                        parts[1],
                        e.value().load(Ordering::Relaxed)
                    );
        ...
        for e in self.histograms.iter() {
            ...
            let _ = writeln!(
                &mut out,
                "fastclaw_provider_latency_ms{{provider=\"{}\",model=\"{}\",quantile=\"0.5\"}} {}",
                parts[0], parts[1], p50
            );
            let _ = writeln!(
                &mut out,
                "fastclaw_provider_latency_ms{{provider=\"{}\",model=\"{}\",quantile=\"0.95\"}} {}",
                parts[0], parts[1], p95
            );
            let _ = writeln!(
                &mut out,
                "fastclaw_provider_latency_ms{{provider=\"{}\",model=\"{}\",quantile=\"0.99\"}} {}",
                parts[0], parts[1], p99
            );
            let _ = writeln!(
                &mut out,
                "fastclaw_provider_latency_ms_sum{{provider=\"{}\",model=\"{}\"}} {}",
                parts[0], parts[1], sum
            );
            let _ = writeln!(
                &mut out,
                "fastclaw_provider_latency_ms_count{{provider=\"{}\",model=\"{}\"}} {}",
