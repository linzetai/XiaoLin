...
async fn list_sessions_includes_created() {
    ...
    state
        .store
        .session_store
        .create_session(&sid1, "main", None)
        .await
        .unwrap();
    state
        .store
        .session_store
        .create_session(&sid2, "main", None)
        .await
        .unwrap();
...
async fn list_sessions_respects_limit() {
    ...
    assert!(
        sessions.len() <= 2,
        "limit should cap results to 2, got {}",
        sessions.len()
    );
}
...
async fn update_session_title_overwrites() {
    ...
    state
        .store
        .session_store
        .update_title(&sid, "First")
        .await
        .unwrap();
    state
        .store
        .session_store
        .update_title(&sid, "Second")
        .await
        .unwrap();
...
async fn config_serialization_roundtrip() {
    ...
    let _parsed: xiaolin_core::config::XiaoLinConfig =
...
async fn full_session_lifecycle() {
    ...
    state
        .store
        .session_store
        .update_title(&sid, "Math Question")
        .await
        .unwrap();
    let s = state
        .store
...
async fn ws_broadcast_delivers_to_subscriber() {
    ...
    let payload =
        json!({"type":"event","event":"sessions.changed","data":{"sessionId":"test-123"}})
            .to_string();
