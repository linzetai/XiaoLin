...
pub fn record_chat_request(agent_id: &str, streaming: bool) {
    ...
    counter!("xiaolin_chat_requests_total", "agent" => sanitize_label(agent_id, 64), "mode" => mode)
        .increment(1);
}

pub fn record_chat_latency(agent_id: &str, start: Instant) {
    let ms = start.elapsed().as_secs_f64() * 1000.0;
    histogram!("xiaolin_chat_latency_ms", "agent" => sanitize_label(agent_id, 64)).record(ms);
}

pub fn record_tool_call(tool_name: &str, success: bool) {
    let ok = if success { "true" } else { "false" };
    counter!("xiaolin_tool_calls_total", "tool" => sanitize_label(tool_name, 64), "success" => ok)
        .increment(1);
}
...
pub fn record_ws_connection(delta: i64) {
    if delta > 0 {
        ...
        gauge!("xiaolin_ws_connections").decrement((-delta) as f64);
    }
}

pub fn record_session_count(count: u64) {
    gauge!("xiaolin_sessions_active").set(count as f64);
}

pub fn record_agent_reload(count: usize) {
    counter!("xiaolin_agent_reloads_total").increment(1);
    gauge!("xiaolin_agents_loaded").set(count as f64);
}

pub fn record_memory_operation(kind: &str) {
    counter!("xiaolin_memory_ops_total", "kind" => kind.to_string()).increment(1);
}

pub fn record_plugin_invocation(plugin_id: &str, success: bool) {
    let ok = if success { "true" } else { "false" };
    counter!("xiaolin_plugin_invocations_total", "plugin" => sanitize_label(plugin_id, 64), "success" => ok).increment(1);
}
...
mod tests {
    ...
    fn render_returns_string_before_init() {
        ...
        assert!(out.is_empty() || out.contains("xiaolin"));
    }
}
