...
- At runtime, FastClaw tries bundled resources first.
- If not found, FastClaw falls back to system `rust-analyzer` in PATH.

CI recommendation:

- Use `.github/scripts/fetch-rust-analyzer.sh <target> <out_dir>` in release pipeline.
- Place resulting binary into `crates/fastclaw-app/src-tauri/resources/lsp/`.
