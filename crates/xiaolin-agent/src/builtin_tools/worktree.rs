...
// ── Worktree-Isolated Task Work Factory ─────────────────────────────

use super::task::TaskWorkFactory;

/// A [`TaskWorkFactory`] wrapper that runs each task in its own git worktree.
///
/// Creates a worktree before delegating to the inner factory, then cleans
/// it up after the task completes (success or failure). This provides
/// filesystem isolation so sub-agents can freely modify files without
/// conflicting with the main working tree.
pub struct WorktreeIsolatedTaskFactory {
    inner: Arc<dyn TaskWorkFactory>,
    base_dir: PathBuf,
}

impl WorktreeIsolatedTaskFactory {
    pub fn new(inner: Arc<dyn TaskWorkFactory>, base_dir: PathBuf) -> Self {
        Self { inner, base_dir }
    }

    async fn setup_worktree(&self, task_id: &str) -> Result<(PathBuf, String), String> {
        if !is_git_repo(&self.base_dir).await {
            return Err("base directory is not a git repository".to_string());
        }
...
#[async_trait]
impl TaskWorkFactory for WorktreeIsolatedTaskFactory {
    async fn run(&self, subject: String, description: String) -> Result<String, String> {
        let task_id = uuid::Uuid::new_v4().to_string();
        let (wt_path, branch) = self.setup_worktree(&task_id).await?;
        ...
        if let Err(e) = std::env::set_current_dir(&wt_path) {
            ...
            return Err(format!("failed to change to worktree dir: {e}"));
        }

        let result = self.inner.run(subject, description).await;
