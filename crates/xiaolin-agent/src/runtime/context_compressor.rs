...
pub struct CompressionResult {
    ...
    pub trigger: xiaolin_core::types::CompactTrigger,
}

impl CompressionResult {
    /// Create a result indicating no compression happened.
    pub(crate) fn no_op(messages: Vec<ChatMessage>) -> Self {
        let estimated = xiaolin_context::estimate_messages_tokens(&messages);
        Self {
            compressed: false,
            ...
            history_file: None,
            messages_removed: 0,
            trigger: xiaolin_core::types::CompactTrigger::Auto,
        }
    }
}
...
fn save_history_file(messages: &[ChatMessage]) -> Option<String> {
    let dir = std::env::temp_dir().join("xiaolin_history");
...
pub async fn try_compress_chat_with_threshold(
    ...
    let local_estimate = xiaolin_context::estimate_messages_tokens(messages);
    ...
    let new_estimated = xiaolin_context::estimate_messages_tokens(&new_messages);
    ...
    let boundary = ChatMessage::compact_boundary(
        xiaolin_core::types::CompactTrigger::Auto,
        estimated,
        new_estimated,
    );
...
impl AutoCompactor {
    ...
    pub async fn compact_if_needed(
        ...
        let estimated = if api_prompt_tokens > 0 {
            ...
            xiaolin_context::estimate_messages_tokens(messages)
        };
