...
pub(crate) struct ProductionDeps {
    ...
    pipeline: Mutex<xiaolin_context::ContextPipeline>,
}
...
pub(crate) mod mock {
    ...
    impl QueryDeps for MockDeps {
        ...
        async fn pre_query_compact(
            ...
            if results.is_empty() {
                let est = xiaolin_context::estimate_messages_tokens(messages);
                UnifiedCompactResult {
...
mod tests {
    ...
    use super::*;
    use xiaolin_core::types::{ChatChoice, ChatMessage, ChatResponse, Role, Usage};
    ...
    async fn production_deps_provider_name() {
        ...
        let pipeline = xiaolin_context::ContextPipeline::new(Default::default());
