...
use xiaolin_core::channel_plugin::{ChannelPluginConfig, ProcessChannelConfig};
    ...
    fn connection_mode(&self) -> &str {
        self.process_config()
            .map(|pc| match pc.transport {
                xiaolin_core::channel_plugin::ProcessTransport::Stdio => "websocket",
                xiaolin_core::channel_plugin::ProcessTransport::Http => "webhook",
            })
            .unwrap_or("unknown")
    }
}
