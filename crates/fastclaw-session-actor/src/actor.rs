...
use fastclaw_protocol::event::AbortReason;
use fastclaw_protocol::id::{SessionId, SubmissionId, TurnId};
...
impl SessionActorConfig {
    ...
    /// since FastClaw sessions are typically lighter.
...
impl SessionActor {
    ...
    async fn handle_user_turn(
        ...
        let handle = tokio::spawn(async move {
            ...
            match outcome {
                Ok(result) => {
                    let summary = fastclaw_protocol::event::TurnSummary {
                        turn_id: task_turn_id.clone(),
        ...
        if self.active_turn.is_some() {
            ...
            self.emit_sync(
                ...
                AgentEvent::Error {
                    ...
                    error_code: Some(fastclaw_protocol::event::ErrorCode::BadRequest),
                },
            );
            return;
        }
