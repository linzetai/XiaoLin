...
pub async fn handle_agents_create(
    ...
    let Ok(config) = serde_json::from_value::<fastclaw_core::agent_config::AgentConfig>(config_val.clone()) else {
        send_resp(
            sender,
            &WsResponse {
                id: req_id,
...
pub async fn handle_agents_update(
    ...
    let Ok(mut config) = serde_json::from_value::<fastclaw_core::agent_config::AgentConfig>(config_val.clone()) else {
        send_resp(
            sender,
            &WsResponse {
                id: req_id,
