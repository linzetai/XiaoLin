...
pub(super) async fn create_plugin(
    State(state): State<AppState>,
    AppJson(config): AppJson<fastclaw_core::llm_plugin::LlmPluginConfig>,
) -> Result<impl axum::response::IntoResponse, AppError> {
    ...
    match config.plugin_type {
        fastclaw_core::llm_plugin::LlmPluginType::Middleware => {
        ...
        fastclaw_core::llm_plugin::LlmPluginType::Process => {
    ...
    // Persist to disk
    let plugins_dir = fastclaw_core::llm_plugin::resolve_plugins_dir(
...
pub(super) async fn update_plugin(
    State(state): State<AppState>,
    Path(plugin_id): Path<String>,
    AppJson(mut config): AppJson<fastclaw_core::llm_plugin::LlmPluginConfig>,
) -> Result<impl axum::response::IntoResponse, AppError> {
