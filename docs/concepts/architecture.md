...
| `fastclaw-model-router` | 模型路由与成本追踪 |
| `fastclaw-context` | 上下文压缩与用户画像 |
...
6. **LLM 与工具循环**：调用模型；若返回 tool_calls，则执行内置 / WASM / MCP 工具并将结果回灌。
7. **出站**：将最终文本（及卡片等）通过对应渠道 SDK 或 WebSocket 事件推回客户端。
