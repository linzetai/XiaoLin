...
编译产物位于 `target/release/fastclaw`。将其复制到 `$PATH` 目录即可：

```bash
sudo cp target/release/fastclaw /usr/local/bin/
```
...
完成后会生成：
- `~/.fastclaw/config/default.json` — 主配置
...
配置文件支持 **JSON5** 格式（允许注释和尾逗号）。路径：`~/.fastclaw/config/default.json`
...
可通过环境变量 `RUST_LOG` 覆盖，例如 `RUST_LOG=fastclaw_gateway=debug,fastclaw_agent=trace`。
...
**嵌入提供商：**
- `local` — 纯 Rust 本地推理（hypembed），无需外部 API，首次使用自动下载模型到 `~/.fastclaw/models/`
...
```json5
{
  "dag": {
    ...
    "checkpointDbPath": "data/dag_checkpoints.db",
    "defaultNodeTimeoutSecs": 60
  }
}
```
...
# Started FastClaw gateway daemon (pid 12345). PID file: ~/.fastclaw/daemon.pid
...
日志写入 `~/.fastclaw/logs/gateway-daemon.log`。

### 4.3 多环境配置

```bash
# 开发环境（数据隔离在 ~/.fastclaw-dev/）
fastclaw --dev serve

# 命名环境
fastclaw --profile staging serve
# 数据在 ~/.fastclaw-staging/
```
...
| `FASTCLAW_STATE_DIR` | 状态目录路径 |
| `OPENAI_API_KEY` | OpenAI API Key |
| `ANTHROPIC_API_KEY` | Anthropic API Key |
| `DASHSCOPE_API_KEY` | DashScope API Key |
| `DEEPSEEK_API_KEY` | DeepSeek API Key |
| `GOOGLE_API_KEY` | Google API Key |
| `FASTCLAW_API_KEYS` | Gateway 认证密钥（逗号分隔） |
...
### 5.2 `fastclaw onboard`

包含 setup 的新手引导，额外展示功能概览和 Quick Start。

### 5.3 `fastclaw serve`

前台启动 Gateway。等价于 `fastclaw gateway run`。
...
fastclaw config get credentials.openai.apiKey

# 设置指定键（自动创建中间对象）
fastclaw config set gateway.port 8080
fastclaw config set logging.level debug
...
### 5.7 `fastclaw gateway`

```bash
fastclaw gateway run       # 前台运行（同 serve）
fastclaw gateway start     # 后台启动（Unix only）
fastclaw gateway stop      # 停止后台进程
fastclaw gateway restart   # 重启后台进程
fastclaw gateway status    # 查看后台进程状态
fastclaw gateway health    # 健康检查
```

### 5.8 `fastclaw sessions`

```bash
# 列出最近会话
fastclaw sessions list
fastclaw sessions list --limit 50 --offset 0

# 查看会话详情
fastclaw sessions get <session_id>

# 删除会话
fastclaw sessions delete <session_id>

# 清理过期会话
fastclaw sessions cleanup
fastclaw sessions cleanup --ttl-hours 72
```
...
# 指定 Gateway URL
fastclaw tui --url ws://192.168.1.100:18789/ws

# 带认证
fastclaw tui --token your-api-key

# 恢复会话
fastclaw tui --session <session_id>
```
...
# 列出对话 Trace
fastclaw trace list
fastclaw trace list --limit 100

# 查看 Trace 详情
fastclaw trace show <trace_id>

# 导出为 JSON
fastclaw trace export <trace_id>
```

### 5.13 `fastclaw backup`

```bash
# 创建备份（使用 VACUUM INTO 保证一致性）
fastclaw backup create
fastclaw backup create --output /path/to/backup/

# 恢复备份
fastclaw backup restore /path/to/backup/
```

备份包含：`sessions.db`、`memory.db`、`evolution.db`、`config.json`。

### 5.14 `fastclaw mcp-server`

启动 MCP Server（stdio 传输），暴露 FastClaw 的内置工具供外部 Agent 调用：

```bash
fastclaw mcp-server
```

可在外部 Agent（如 Claude Desktop、Cursor）中将 FastClaw 配置为 MCP Server。

### 5.15 `fastclaw completions`
...
# Fish
fastclaw completions fish > ~/.config/fish/completions/fastclaw.fish

# PowerShell
fastclaw completions powershell > _fastclaw.ps1
```
...
```bash
kill -HUP $(cat ~/.fastclaw/daemon.pid)
```
...
FastClaw 将工具按操作性质分为以下类别，用于并发调度：
...
本地嵌入使用 hypembed（纯 Rust 实现），首次使用自动下载模型到 `~/.fastclaw/models/`。支持 HuggingFace 上的 sentence-transformers 系列模型。
...
FastClaw 可以作为 MCP Server 运行，向外部 Agent 暴露其内置工具：
...
```json
{
  "mcpServers": {
    "fastclaw": {
      "command": "/usr/local/bin/fastclaw",
      "args": ["mcp-server"]
    }
  }
}
...
RUST_LOG=fastclaw_gateway=trace,fastclaw_agent=debug fastclaw serve
```
...
```bash
# 自动备份到 ~/.fastclaw/backups/<timestamp>/
fastclaw backup create

# 指定输出目录
fastclaw backup create --output /mnt/backup/fastclaw/
```
...
# 部署到服务器
scp target/release/fastclaw user@server:/usr/local/bin/
...
```bash
# 创建 ConfigMap
kubectl create configmap fastclaw-config --from-file=config/default.json

# 创建 Secret
kubectl create secret generic fastclaw-secrets \
...
原因：未配置 LLM API Key。解决：通过 `fastclaw config set credentials.xxx.apiKey sk-xxx` 设置，或设置对应环境变量。
...
原因：Gateway 未启动或未运行过。解决：先启动 `fastclaw serve`，数据库会自动创建。
...
原因：已有后台进程在运行。解决：先 `fastclaw gateway stop`，再重新启动。
...
# 特定模块的 trace 日志
RUST_LOG=fastclaw_agent=trace fastclaw serve
