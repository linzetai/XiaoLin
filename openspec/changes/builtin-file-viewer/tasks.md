...
- [ ] 6.1 创建 `ImageViewer.tsx`：通过新增 `read_binary_for_viewer` IPC 命令加载图片为 base64 → blob URL（复用 `read_file_for_viewer` 的路径安全校验，但返回二进制而非文本）
- [ ] 6.2 实现缩放功能：鼠标滚轮缩放（以鼠标位置为锚点），10%-500% 范围
- [ ] 6.3 实现拖拽平移：放大后鼠标拖拽移动
- [ ] 6.4 实现工具栏控制：适应窗口、原始大小、当前缩放比例显示
- [ ] 6.5 SVG 文件支持：图片预览/源码查看切换
