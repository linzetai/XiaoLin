...
- `crates/xiaolin-sandbox/src/seatbelt.rs` — 策略生成修复
- `crates/xiaolin-sandbox/src/lib.rs` — `SandboxedCommand` working_dir 传递
- `crates/xiaolin-sandbox/src/landlock.rs` — 同上
- `crates/xiaolin-agent/src/runtime/runtimes/shell.rs` — 核心执行逻辑重构
- `crates/xiaolin-agent/src/builtin_tools/shell.rs` — schema 清理
- `crates/xiaolin-agent/src/runtime/orchestrator.rs` — escalation 逻辑增强
- 无 API 变更、无外部依赖变更
