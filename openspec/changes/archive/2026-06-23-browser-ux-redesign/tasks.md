...
- [x] 10.1 验证全宽模式下浏览器位于左侧、Chat 位于右侧
- [x] 10.2 验证进度条在页面导航时正确显示和消失（含 15s trickling）
- [x] 10.3 验证 Stop 按钮能停止页面加载，前端不卡在 loading 状态
- [x] 10.4 验证后退/前进按钮有即时反馈，慢速网络下不误恢复 ready
- [x] 10.5 验证 Chat 折叠时完全隐藏、通过 toggle 按钮可展开
- [x] 10.6 验证 Chat 折叠/展开后消息状态保持（不重挂载）
- [x] 10.7 验证拖拽调整 Chat 宽度方向正确（向左拖=变宽）
- [x] 10.8 验证选区发送 Chat 时自动展开折叠的 Chat 面板
- [x] 10.9 验证 Panel 模式未受影响（布局、交互保持不变）
- [x] 10.10 验证快捷键切回 Panel 模式后 WebView 状态保持
- [x] 10.11 验证 Favicon 在页面加载后正确显示、导航时重置、加载失败回退 Globe
- [x] 10.12 验证新增键盘快捷键功能正确：Ctrl+Tab 切换标签、Ctrl+1~8 跳转、F5 刷新、Escape 停止
- [x] 10.13 验证快捷键作用域：Panel 模式 Files tab 时 Ctrl+Tab 不触发；Chat Composer 聚焦时 F5/Escape 不拦截；全宽模式浏览器可见时正常工作
