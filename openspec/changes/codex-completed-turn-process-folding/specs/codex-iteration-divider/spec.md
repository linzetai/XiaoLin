...
Iteration boundaries SHALL be represented in the canonical timeline but SHALL NOT render as user-facing labels in the default Codex App / ChatGPT-like chat UI, the default completed view, or the expanded completed process transcript unless diagnostic mode is explicitly enabled.
...
#### Scenario: Completed process expands without labels
- **WHEN** the user expands a completed process transcript
- **THEN** iteration boundary labels SHALL remain hidden in the user-facing transcript
