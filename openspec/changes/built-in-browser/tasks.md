...
- [ ] 5.1 `crates/xiaolin-tools-browser/src/engine/mod.rs`：定义 BrowserEngine trait
- [ ] 5.2 `engine/cdp_engine.rs`：封装现有 headless_chrome 为 CdpEngine
- [ ] 5.3 `engine/webview_engine.rs`：TauriWebViewEngine 通过 AppHandle 操作内置 WebView
- [ ] 5.4 重构 BrowserTool：从直接使用 headless_chrome 改为 BrowserEngine trait
- [ ] 5.5 迁移导航类 actions（navigate、go_back、go_forward、reload）
- [ ] 5.6 迁移交互类 actions（click、fill、fill_form、type_text、press_key、hover、scroll、select）→ JS injection + Agent Control Mode 联动
- [ ] 5.7 迁移快照类 actions（take_snapshot、get_content）→ WebView eval + `untrusted_webpage` 标记
- [ ] 5.8 迁移截图 action → 平台原生 API + html2canvas fallback
- [ ] 5.9 迁移页面管理 actions（list_pages、select_page、new_page、close_page）→ BrowserPanelManager
- [ ] 5.10 迁移 DevTools actions → custom protocol 回传 / initialization_script 捕获数据
- [ ] 5.11 迁移 cookies action → `document.cookie`（仅非 HttpOnly）
- [ ] 5.12 迁移 wait_for → JS 轮询
- [ ] 5.13 迁移 drag、handle_dialog、interact、emulate、resize_page、pdf、upload_file
- [ ] 5.14 操作可视化高亮 JS（Layer 5, eval 注入）
- [ ] 5.15 Agent Control Mode 拦截 JS（initialization_script 中的 capture phase 事件监听）
- [ ] 5.16 gateway 启动逻辑分支：Tauri → TauriWebViewEngine，纯 gateway → CdpEngine
