...
3. ~~**三套 Transport 重载逻辑不一致**~~ → ✅ T4+T5 完成：`connect_mcp_server` 统一入口，三条路径均已迁移
...
7. ~~**Notification 黑洞**~~ → ✅ T6 完成：三条 reader loop 均增加 notification dispatch（broadcast channel），`subscribe_notifications()` + `refresh_tools()` 公开 API
...
| Transport 枚举 + `connect_mcp_server` | ✅ | T4，`McpTransportType` 枚举 + `connect_mcp_server` 统一入口 |
| 启动/热重载路由统一 | ✅ | T5，三条路径统一调用 `connect_mcp_server` |
| Notification dispatch | ✅ | T6，broadcast channel + 三条 reader loop 分流 + `subscribe_notifications()` |
