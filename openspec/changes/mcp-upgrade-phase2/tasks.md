...
- [x] 2.5 验证 — mock MCP server 测试通过：`progress_notification_dispatched_to_subscriber` + `call_tool_with_progress_injects_meta_token`
...
- [x] 3.4 验证 — `resources_list_changed_notification_dispatched` + `prompts_list_changed_notification_dispatched` 通过
...
- [x] 4.5 验证 — `register_client_with_mock_dcr_endpoint` + `register_client_dcr_failure_returns_error` + `file_token_store_save_load_delete` 通过
...
- [x] 6.4 验证 — `roots_list_request_dispatched_as_server_request` 通过
...
- [x] 7.5 验证 — `create_xiaolin_mcp_server_filters_mcp_prefix` + `create_xiaolin_mcp_server_tool_call_works` 通过
...
- [x] 8.7 验证 — `websocket_mcp_client_connect_and_tools_list` + `websocket_mcp_client_tool_call` + `websocket_notification_received` 通过
