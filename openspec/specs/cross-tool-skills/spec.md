...
- **AND** they SHALL have lower priority than `.xiaolin/skills/`

#### Scenario: Discover Cursor user skills
- **WHEN** `~/.cursor/skills/` contains skills
- **THEN** they SHALL be discovered at user level with lower priority than `~/.xiaolin/skills/`
...
- **THEN** it SHALL only write to `.xiaolin/skills/` (never to `.cursor/` or `.codex/`)

### Requirement: Skill source tracking
- **WHEN** skills are discovered from multiple sources
- **THEN** each skill SHALL carry source metadata: `{ origin: "xiaolin"|"cursor"|"codex"|"shared", layer: "project"|"user", path }`
...
4. `~/.xiaolin/skills/` (XiaoLin user)
5. `<root>/skills/` (legacy project)
6. `<root>/.cursor/skills/` (Cursor project, read-only)
7. `<root>/.xiaolin/skills/` (XiaoLin project, highest priority)
